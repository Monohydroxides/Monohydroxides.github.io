...
公式推导过程如下。
有 $\left| Good_i \right| = \prod C_k, (i = p_1^{C_1} \cdot p_2^{C_2} \cdot \dots \cdot p_k^{C_k})$，
...
上式中，$\sum_{n = 1}^{m} g(\frac{n}{d}) [d \mid n] = \sum_{n = 1}^{\frac{m}{d}} g(n) = \frac{(1 + \frac{m}{d}) \cdot \frac{m}{d}}{2}$，
又 $f(p) = h(1)g(p) + h(p)g(1) = ph(1) + h(p) = p$，且 $h(p = 0)$，
利用结论 $\left| PN \right| \sim O(\sqrt{N})$，
有 $ans = \frac{1}{m} \sum_{d \in PN} h(d) \cdot \frac{(1 + \frac{m}{d}) \cdot \frac{m}{d}}{2}$。
